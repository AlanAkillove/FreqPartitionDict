...
为指导用户选择最优配置参数，我们系统测试了晋升阈值和热区容量对性能的影响。

#### 10.5.1 晋升阈值影响

| 阈值 | 命中率 | 晋升次数 | 降级次数 | 说明 |
|------|--------|----------|----------|------|
| 1 | 100% | 23,315 | 23,187 | 快速适应，高晋升率 |
| 2 | 100% | 17,471 | 17,343 | 平衡适应速度 |
| **3** | **100%** | **14,345** | **14,217** | **推荐默认值** |
| 5 | 100% | 10,600 | 10,472 | 更稳定，适应较慢 |
| 10 | 100% | 6,776 | 6,648 | 高稳定性 |
| 20 | 100% | 3,233 | 3,105 | 极稳定，适应慢 |

*表 8：晋升阈值影响（α = 1.2，H = 128，100,000 次操作）。*
...
*表 9：热区容量影响（α = 1.2，阈值 = 3，N = 10,000）。*
...
为验证 FreqPartitionDict 在长期运行中的稳定性，我们执行了 1,000,000 次操作的连续测试。
...
| 最小命中率 | 100.00% | 全程稳定 |
| 最大命中率 | 100.00% | 无波动 |
| 命中率标准差 | 0.00% | EXCELLENT |
| 晋升/降级比 | 1.00 | 完美平衡 |
| 吞吐量 | 1,475 ops/sec | 稳定性能 |

*表 10：长期稳定性测试结果。*

**关键发现**：
1. 命中率在整个测试期间保持 100%，无波动
2. 晋升和降级操作完美平衡，表明热区管理健康
3. 内存使用稳定，无泄漏迹象

### 10.7 批量操作优化

为提高批量数据处理效率，FreqPartitionDict 提供了批量操作接口：
...
**线程安全版本优势**：批量操作在单个锁保护下执行，显著减少锁竞争：

| 操作类型 | 单次操作 | 批量操作（100项） | 加速比 |
|----------|----------|-------------------|--------|
| 插入 | 125 ns | 12 ns/项 | 10.4× |
| 查询 | 275 ns | 25 ns/项 | 11.0× |
| 删除 | 90 ns | 8 ns/项 | 11.3× |

*表 11：批量操作性能对比（线程安全版本）。*
